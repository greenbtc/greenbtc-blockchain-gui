...
import { AlertDialog, Card, Flex, greenBTCToMojo, mojoToGreenBTCLocaleString } from '@greenbtc/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const greenbtcper_input = ip_parsed.limit;
    ...
    const greenbtcper_value = Number.parseInt(Number(greenbtcper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (greenbtc per interval):{' '}
                  {mojoToGreenBTCLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToGreenBTCLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = greenBTCToMojo(fee_input.value);
