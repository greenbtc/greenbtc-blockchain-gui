import {TransactionType, TransactionTypeFilterMode, StakeValue, toBech32m} from '@greenbtc-network/api';
import {
  AddressBookContext,
  Card,
  CopyToClipboard,
...
import type { Row } from '@greenbtc-network/core';
...
const stakeDisplayName = (val: StakeValue, isStakeFarm: boolean): string => {
...
const getCols = (isStakeFarm: boolean) => [
  {
    field: (row: Row, metadata: any) => (
        <>
          <strong>
            <FormatLargeNumber value={mojoToGreenBTC(row.amount)} />
          </strong>
          &nbsp;
          {metadata.feeUnit}
        </>
...

  {
    field: (row: Row, metadata) => (
      ...
      <Typography color="textPrimary" variant="body2">
        {stakeDisplayName(metadata.stakeList[row.metadata?.stakeType] || {timeLock:0, coefficient:0}, isStakeFarm)}
      </Typography>
        <Flex>
          {row.confirmed ? (
