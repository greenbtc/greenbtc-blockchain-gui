...
import {mojoToGreenBTC} from "@greenbtc-network/core/src";
...
export default function StakeSend(props: SendCardProps) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    const amountMojo = greenbtcToMojo(amount);
    if (amountMojo < stakeMin) {
      throw new Error(t`stake min ${mojoToGreenBTC(stakeMin)}`);
    }
          ...
          <Grid xs={12} item>
            ...
            <Grid xs={12} md={4} item>
              <EstimatedFee
                ...
                fullWidth
                txType={FeeTxType.walletSendGBTC}
              />
            </Grid>
          </Grid>
        </Card>
