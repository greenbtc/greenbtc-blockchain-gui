...
import { AlertDialog, Card, Flex, Loading, Dropzone, mojoToGreenBTCLocaleString } from '@greenbtc/core';
import {
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToGreenBTCLocaleString(props.balance)} TGBTC
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const CreateAttest = (props) => {
  ...
  function createAttestPacket() {
    ...
    if (address.substring(0, 12) === 'greenbtc_addr://') {
...
const CashoutCard = (props) => {
  ...
  function cashout() {
    ...
    if (puzzlehash.slice(0, 12) === 'greenbtc_addr://') {
