...
import { AlertDialog, greenBTCToMojo } from '@greenbtc/core';
import {
  Typography,
  Button,
  Box,
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      greenbtcper_input.value === '' ||
      Number(greenbtcper_input.value) === 0 ||
      !Number(greenbtcper_input.value) ||
      isNaN(Number(greenbtcper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const greenbtcper = greenBTCToMojo(greenbtcper_input.value);
    const greenbtcper_value = Number.parseInt(Number(greenbtcper));
    const userpubkey = userpubkey_input.value;
    const amount = greenBTCToMojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = greenBTCToMojo(fee_input.value);
