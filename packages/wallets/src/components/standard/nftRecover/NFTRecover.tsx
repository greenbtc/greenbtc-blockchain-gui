import type { NFTRecoverInfo } from '@greenbtc-network/api';
import {
  useLocalStorage,
...
export default function NFTRecover(props: NFTRecoverProps) {
  ...
  async function handleSubmit() {
    ...
    const response = await recoverPoolNFT({launcherId, contractAddress}).unwrap();
    const t1 = t`Recovered Amount:`;
    const t2 = t`Status:`;
    const t3 = response.status === 'SUCCESS' ? t`SUCCESS` : t`FAILED`;
    setContent(`${t1  } ${  mojoToGreenBTC(response.amount)  } ${  currencyCode}, ${t2  } ${t3
        ...
        <Grid spacing={2} alignItems="stretch" container>
          ...
          <Grid xs={12} md={4} item>
            <CardSimple
              ...
              value={nftData?mojoToGreenBTCLocaleString(nftData.totalAmount, locale):"-"}
            />
          </Grid>
          <Grid xs={12} md={4} item>
            <CardSimple
              ...
              value={nftData?mojoToGreenBTCLocaleString(nftData.balanceAmount, locale):"-"}
            />
          </Grid>
          <Grid xs={12} md={4} item>
            <CardSimple
              ...
              value={nftData?mojoToGreenBTCLocaleString(nftData.recordAmount, locale):"-"}
            />
          </Grid>
        </Grid>
        </Form>
