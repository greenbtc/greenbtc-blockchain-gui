import { useGetFarmedAmountQuery } from '@greenbtc-network/api-react';
import {
  useCurrencyCode,
...
function FarmingRewardsHistoryCards() {
  ...
  const totalGreenBTCFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total GBTC Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    if (data && data.farmedAmount) {
        ...
        <StyledTable>
          <tbody>
            <tr>
              ...
              <td>
                {mojoToGreenBTCLocaleString(data.poolRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToGreenBTCLocaleString(data.farmerRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToGreenBTCLocaleString(data.feeAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
      ...
      cardValue = (
        <Tooltip title={title}>
          {mojoToGreenBTCLocaleString(data.farmedAmount, locale)}
          &nbsp;
          {currencyCode}
        </Tooltip>
      );
    ...
    return <CardSimple title={<Trans>Total GBTC Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
