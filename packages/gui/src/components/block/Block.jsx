import { toBech32m } from '@greenbtc-network/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@greenbtc-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToGreenBTC(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToGreenBTC(calculateBaseFarmerReward(blockRecord.height));

  const greenbtcFees = blockRecord.fees !== undefined ? mojoToGreenBTC(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: greenbtcFees ? `${greenbtcFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the GreenBTC blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
