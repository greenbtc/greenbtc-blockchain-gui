...
import { toBech32m } from '@greenbtc/api';
import { useGetBlockQuery, useGetBlockRecordQuery } from '@greenbtc/api-react';
...
export default function Block() {
  ...
  const baseFarmerReward = mojoToGreenBTC(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const greenbtcFees =
    blockRecord.fees !== undefined ? mojoToGreenBTC(blockRecord.fees) : '';
    ...
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>
              Block at height {blockRecord.height} in the GreenBTC blockchain
            </Trans>
          </Back>
        }
        action={
