import { Flex, Loading, TooltipIcon } from '@greenbtc-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `greenbtc_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.greenbtc?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the GreenBTC application may not support some commands requested by the
