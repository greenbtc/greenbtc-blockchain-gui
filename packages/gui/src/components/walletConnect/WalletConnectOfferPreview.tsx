import { Button, Flex, useOpenDialog, greenbtcToMojo } from '@greenbtc-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeGreenBTC = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeGreenBTC) {
        const feeMojos = feeGreenBTC ? greenbtcToMojo(feeGreenBTC).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
