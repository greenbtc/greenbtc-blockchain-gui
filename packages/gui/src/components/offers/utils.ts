...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@greenbtc-network/api';
import { mojoToCAT, mojoToGreenBTC, mojoToCATLocaleString, mojoToGreenBTCLocaleString } from '@greenbtc-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> GBTC assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToGreenBTCLocaleString(amount, locale);
  }
...
export function offerAssetTypeForAssetId(assetId: string, offerSummary: OfferSummaryRecord): OfferAsset | undefined {
  ...
  if (['gbtc', 'tgbtc'].includes(assetId)) {
    assetType = OfferAsset.GREENBTC;
  } else {
    const { infos } = offerSummary;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.GREENBTC) {
    return keys.includes('gbtc') ? 'gbtc' : undefined;
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.GREENBTC]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.GREENBTC ? mojoToGreenBTC(amountInMojos) : mojoToCAT(amountInMojos);
