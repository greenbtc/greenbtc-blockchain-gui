...
import { OfferTradeRecord } from '@greenbtc/api';
import {
  Button,
  Checkbox,
  Dialog,
...
const log = debug('greenbtc-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.greenbtc.top';
...
async function postToOfferBin(
  ...
  if (testnet) {
    return 'https://www.greenbtc.top/offers';
  }
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50029, // Pairing doesn't exist e.g. GBTC/RandoCoin
...
async function postToSpacescan(
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tgbtc' : 'gbtc'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
  ...
  return `https://www.spacescan.io/${testnet ? 'tgbtc' : 'gbtc'}/offer/${id}`;
}
...
const KeybaseTeamName = 'greenbtc_offers';
