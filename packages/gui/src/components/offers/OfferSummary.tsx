import { type OfferSummaryRecord } from '@greenbtc-network/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToGreenBTC, mojoToCAT } from '@greenbtc-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['gbtc', 'tgbtc'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToGreenBTC(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['gbtc', 'tgbtc'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToGreenBTC(takerEntries[0][1])
