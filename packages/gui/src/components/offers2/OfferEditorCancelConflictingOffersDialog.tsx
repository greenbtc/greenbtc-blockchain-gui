import { ConfirmDialog, Flex } from '@greenbtc-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'GBTC' && assetStatus.type === 'GBTC') {
      // assetId might be either 'GBTC' or 'UNKNOWN'(maybe fee) but here both are treated as just a GBTC spending
