...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (GBTC), indicates whether a token or CAT has a default entry
 ...
 * for creating an GBTC offer, a CAT offer, or an NFT offer. GBTC and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      gbtc: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
