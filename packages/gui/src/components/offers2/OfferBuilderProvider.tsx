import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@greenbtc-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@greenbtc-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@greenbtc-network/api-react';
import { catToMojo, greenbtcToMojo } from '@greenbtc-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedGBTC = useWatch({
    name: 'requested.gbtc',
  });

  const offeredGBTC = useWatch({
    name: 'offered.gbtc',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedGBTC ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('gbtc', greenbtcToMojo(amount))),
    ...(requestedTokens ?? [])
