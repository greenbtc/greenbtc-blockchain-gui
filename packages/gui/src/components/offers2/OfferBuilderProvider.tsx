...
import type { CalculateRoyaltiesRequest, NFTInfo } from '@greenbtc/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@greenbtc/api-react';
import { catToMojo, greenBTCToMojo } from '@greenbtc/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedFungibleAssets = [
    ...(requestedGBTC ?? [])
      ...
      .map(({ amount }) =>
        fungibleAssetFromAssetIdAndAmount('gbtc', greenBTCToMojo(amount)),
      ),
    ...(requestedTokens ?? [])
