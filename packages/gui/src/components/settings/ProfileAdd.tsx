...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'GBTC').toUpperCase();
  const isTestnet = currencyCode === 'TGBTC';
  ...
  const spendableBalance = mojoToGreenBTCLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.greenbtc.top' : 'faucet.greenbtc.top'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the GreenBTC Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
