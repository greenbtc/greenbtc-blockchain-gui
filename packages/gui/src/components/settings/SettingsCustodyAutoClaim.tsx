import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@greenbtc-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, greenbtcToMojo, mojoToGreenBTC } from '@greenbtc-network/core';
import { ConnectCheckmark } from '@greenbtc-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToGreenBTC(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = greenbtcToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
