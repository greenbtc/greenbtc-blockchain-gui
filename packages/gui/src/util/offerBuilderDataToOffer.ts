...
import { greenBTCToMojo, catToMojo } from '@greenbtc/core';
...
export default async function offerBuilderDataToOffer(
  ...
  const hasOffer =
    !!offeredGbtc.length || !!offeredTokens.length || !!offeredNfts.length;
  const hasRequest =
    !!requestedGbtc.length || !!requestedTokens.length || !!requestedNfts.length;
  ...
  await Promise.all(
    offeredGbtc.map(async (gbtc) => {
      ...
      if (!hasEnoughBalance) {
        throw new Error(t`Amount exceeds GBTC spendable balance`);
      }
    }),
  );
