...
import { greenbtcToMojo, catToMojo } from '@greenbtc-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { gbtc: offeredGbtc = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { gbtc: requestedGbtc = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? greenbtcToMojo(firstFee.amount) : new BigNumber(0);

  const walletIdsAndAmounts: Record<string, BigNumber> = {};
  const driverDict: Record<string, Driver> = {};

  if (!allowEmptyOfferColumn) {
    const hasOffer = !!offeredGbtc.length || !!offeredTokens.length || !!offeredNfts.length;
        ...
        if (assetId.toUpperCase() === 'GBTC' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have gbtc spending which is only GBTC, only Fee or both GBTC and Fee
              pendingOffers[idx].assetId = 'GBTC+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'GBTC',
  ...
  let pendingGbtcOffer: AssetStatusForOffer | undefined;
  let pendingGbtc = new BigNumber(0);
  if (offeredGbtc.length > 0 || feeInMojos.gt(0)) {
  ...
  // offeredGbtc.length should be always 0 or 1
  const gbtcTasks = offeredGbtc.map(async (gbtc) => {
    const { amount } = gbtc;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an GBTC amount`);
    }
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }

    const mojoAmount = greenbtcToMojo(amount);
    ...
    const hasEnoughTotalBalance = spendableBalance.plus(pendingGbtc).minus(feeInMojos).gte(mojoAmount);
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds GBTC total balance`);
    }

    if (pendingGbtcOffer) {
      // Assuming offeredGbtc.length is always less then or equal to 1
      pendingGbtcOffer.spendingAmount = mojoAmount.plus(feeInMojos);
      pendingGbtcOffer.spendableAmount = spendableBalance;
      pendingGbtcOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingGbtcOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
        pendingGbtcOffer.status = 'conflictsWithNewOffer';
      } else {
        pendingGbtcOffer.status = 'alsoUsedInNewOfferWithoutConflict';
      }
    }
  });
  // Treat fee as gbtc spending
  if (offeredGbtc.length === 0 && feeInMojos.gt(0)) {
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds GBTC total balance`);
    }
    if (pendingGbtcOffer) {
      pendingGbtcOffer.spendingAmount = feeInMojos;
      pendingGbtcOffer.spendableAmount = spendableBalance;
      pendingGbtcOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingGbtcOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
  ...
  await Promise.all([...gbtcTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedGbtc.forEach((gbtc) => {
    const { amount } = gbtc;
    ...
    walletIdsAndAmounts[wallet.id] = greenbtcToMojo(amount);
  });
  ...
  const prepareNftTasks = requestedNfts.map(async ({ nftId }) => {
    ...
    if (driver) {
      ...
      if (considerNftRoyalty && pendingGbtcOffer) {
        ...
        if (royaltyPercentageStr) {
          ...
          const spendingGbtc = pendingGbtcOffer.spendingAmount.minus(feeInMojos);
          const newSpendingGbtc = spendingGbtc.multipliedBy(royaltyMultiplier).plus(feeInMojos);
          pendingGbtcOffer.spendingAmount = newSpendingGbtc;

          const hasEnoughSpendableBalance = pendingGbtcOffer.spendableAmount.gte(pendingGbtcOffer.spendingAmount);
          if (!hasEnoughSpendableBalance) {
            pendingGbtcOffer.status = 'conflictsWithNewOffer';
          } else {
            pendingGbtcOffer.status = 'alsoUsedInNewOfferWithoutConflict';
