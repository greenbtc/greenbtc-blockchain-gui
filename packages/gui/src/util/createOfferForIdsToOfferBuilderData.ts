import { toBech32m, WalletType } from '@greenbtc-network/api';
import { mojoToGreenBTC, mojoToCAT } from '@greenbtc-network/core';
...
export default function createOfferForIdsToOfferBuilderData(
      ...
      if (asset) {
        switch (asset.walletType) {
          case WalletType.STANDARD_WALLET:
            section.gbtc.push({ amount: mojoToGreenBTC(numericValue.abs()).toFixed() });
            break;
          case WalletType.CAT:
  ...
  if (fee) {
    offerBuilderData.offered.fee = [{ amount: mojoToGreenBTC(fee).toFixed() }];
  }
