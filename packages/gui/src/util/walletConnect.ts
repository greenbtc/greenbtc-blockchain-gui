...
const log = initDebug('greenbtc-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `greenbtc_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.greenbtc;
    if (!requiredNamespace) {
      throw new Error('Missing required greenbtc namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['greenbtc:testnet', 'greenbtc:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `greenbtc:${instance}:${fingerprint}`);
