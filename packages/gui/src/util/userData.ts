...
export function getUserDataDir(): string {
  const greenbtcRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(greenbtcRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const greenbtcRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${greenbtcRootUserDataPath}`);
  app.setPath('userData', greenbtcRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(greenbtcRootUserDataPath, 'Local Storage', 'leveldb');
