...
const PY_DIST_FILE = 'greenbtc_daemon';
const PY_FOLDER = '../../../greenbtc/daemon';
...
const getGreenBTCVersion = () => {
  let version = null;
  const exePath = getExecutablePath('greenbtc');
  // first see if we can get a greenbtc exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or greenbtc is on the path
    try {
      version = childProcess
