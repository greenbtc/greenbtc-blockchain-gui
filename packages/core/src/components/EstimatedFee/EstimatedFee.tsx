import { useGetFeeEstimateQuery } from '@greenbtc-network/api-react';
...
import mojoToGreenBTCLocaleString from '../../utils/mojoToGreenBTCLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendGBTC = 'send_gbtc_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToGreenBTCLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const gbtcFee = mojoToGreenBTCLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, gbtcFee);
        }
      }
    }
