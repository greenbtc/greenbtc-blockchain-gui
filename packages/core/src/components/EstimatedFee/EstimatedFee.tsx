...
import { useGetFeeEstimateQuery } from '@greenbtc/api-react';
import {
  Fee,
  Flex,
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const txCostEstimates = {
    walletSendGBTC: Math.floor(maxBlockCostCLVM / 1170),
  ...
  function formatEst(number, multiplier, locale) {
    ...
    let formatNum = mojoToGreenBTCLocaleString(num, locale);
    return formatNum;
  }
